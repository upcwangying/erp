...
function _create(expr, options) {
	...
	options.uploadJson = _undef(options.uploadJson, options.basePath + 'jsp/upload_json.jsp');
	options.themesPath = _undef(options.themesPath, options.basePath + 'themes/');
	options.langPath = _undef(options.langPath, options.basePath + 'lang/');
	options.pluginsPath = _undef(options.pluginsPath, options.basePath + 'plugins/');
	options.imageSizeLimit = _undef(options.imageSizeLimit, '2MB');
	options.imageUploadLimit = _undef(options.imageUploadLimit, 10);
...
_plugin('core', function(K) {
	...
	self.beforeSetHtml(function(html) {
		...
		.replace(/<a[^>]*name="([^"]+)"[^>]*>(?:<\/a>)?/ig, function(full) {
			...
			return '<img class="ke-anchor" src=' + self.themesPath + '"themes/common/anchor.gif" data-ke-name="' + escape(attrs.name) + '" />';
		})
...
KindEditor.plugin('multiimage', function(K) {
	var self = this, name = 'multiimage',
		...
		imageUploadLimit = self.imageUploadLimit,
		filePostName = K.undef(self.filePostName, 'imgFile'),
		lang = self.lang(name + '.');
